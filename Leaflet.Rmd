...
set.seed(88)
dfx <- data.frame(lat = runif(20, min = 39.25, max = 39.35),
                 lng = runif(20, min = -76.65, max = -76.55),
                 col = sample(c("red", "blue", "green"), 20, replace = TRUE),
                 stringsAsFactors = FALSE)
...
dfx %>%
  leaflet() %>%
  addTiles() %>%
  addCircleMarkers(color = dfx$col) %>%
  addLegend(labels = LETTERS[1:3], colors = c("blue", "red", "green"))
```
...
## Personal tries : Metro stations in Paris
...
dfm <- read.csv(fpath, sep=";", stringsAsFactors = FALSE)
# head(dfm)

dfm2 <- subset(dfm,TYPE == "metro")
# tail(dfm2)
# str(dfm2)
...
# essai couleurs
colstat <-  sample(c("red", "blue", "green"), numstat, replace = TRUE)

df <- data.frame(lat = dfm2$LATITUDE,
                 lng = dfm2$LONGITUDE,
                 popup=dfm2$NOM1,
                 stringsAsFactors = FALSE)
                 # popup=as.character(dfm2$NOM1))
# head(df)
# str(df)
...
df %>% 
  leaflet() %>%
  addTiles() %>%
  addMarkers(popup=df$popup,label = df$popup)
```
